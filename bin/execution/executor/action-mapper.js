...
export default new class {
    ...
    #mapToExecutable(action, location, runtime) {
        return {
            name: action.name,
            group: action.group,
            optional: action.optional ?? this.#getValue(action, 'optional', runtime) ?? false,
            runOnce: action.runOnce ?? this.#getValue(action, 'runOnce', runtime) ?? false,
            hasRun: action.hasRun ?? false,
            lastHash: action.lastHash ?? null,
            location: location.partial,
            elevated: action.elevated ?? this.#getValue(action, 'elevated', runtime) ?? false,
